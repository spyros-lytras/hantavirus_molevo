...
setwd("./") #Change directory as appropriate
#######
#Insert path to log file for the inferred short-term substitution rate (*.log output file from BEAST v1.10.4)
filepath_rates <- "../Rate_posterior_distributions/S_Clade2_strict_priors_1784041387995.log"
filepath_rates <- "../Rate_posterior_distributions/M_Clade2_strict_Priors_1783945744017.log"
filepath_rates <- "../Rate_posterior_distributions/L_Clade2_strict_priors_1783703990177.log"
...
filepath_distances <- "../ultrametric_trees/S_Clade1-4_unclassified_GHOST_v2_Chile2011_BETS_ultrametric_1787243065451.trees"
filepath_distances <- "../ultrametric_trees/M_Clade1-4_unclassified_GHOST_BETS_ultrametric_1787237057098.trees"
filepath_distances <- "../ultrametric_trees/L_Clade1-4_unclassified_GHOST_v2_Chile2011_BETS_ultrametric_1787415193097.trees"

filepath_HKYsubstitutionModel <- "../ultrametric_trees/S_Clade1-4_unclassified_GHOST_v2_Chile2011_BETS_ultrametric_1787243065451.log"
filepath_HKYsubstitutionModel <- "../ultrametric_trees/M_Clade1-4_unclassified_GHOST_BETS_ultrametric_1787237057098.log"
filepath_HKYsubstitutionModel <- "../ultrametric_trees/L_Clade1-4_unclassified_GHOST_v2_Chile2011_BETS_ultrametric_1787415193097.log"
...
meanrate_column <- which(strsplit(con[119],"\t")[[1]]=="clockRate") #For S
meanrate_column <- which(strsplit(con[78],"\t")[[1]]=="clockRate") # For M
meanrate_column <- which(strsplit(con[78],"\t")[[1]]=="clockRate") # For L

#find the position of the kappa and ACTG base frequencies or manually enter their median values below
kappa_column <- which(strsplit(con3[361],"\t")[[1]]=="kappa") #For S
kappa_column <- which(strsplit(con3[352],"\t")[[1]]=="kappa") #For M
kappa_column <- which(strsplit(con3[794],"\t")[[1]]=="kappa") #For L
frequencies1_column <- which(strsplit(con3[361],"\t")[[1]]=="freqParameter.1")
frequencies1_column <- which(strsplit(con3[352],"\t")[[1]]=="freqParameter.1")
frequencies1_column <- which(strsplit(con3[794],"\t")[[1]]=="freqParameter.1")
...
write.table(recreated_treefile, "../recreated_trees/2026-08-24_L_ghost_unclassified_sampled_trees_fixed_mu.trees", sep = "\n", quote = FALSE, row.names = FALSE)

#import <sampled_trees.trees> created in the previous step from the directory
filename <- "../recreated_trees/2026-08-24_L_ghost_unclassified_sampled_trees_fixed_mu.trees"
...
for(el in 1:sample_size){
  ...
  allBranches=base::mapply(c,importedtree[[el]]$edge[,1],importedtree[[el]]$edge[,2],(importedtree[[el]]$edge.length),SIMPLIFY = FALSE)
...
ape::write.nexus(importedtree, file="../PoW_transformed_trees/2026-08-24_L_ghost_unclassified_PoWtransformed_fixed_mu.trees")
