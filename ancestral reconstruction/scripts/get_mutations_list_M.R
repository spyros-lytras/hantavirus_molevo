...
state1 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state1<- state1[state1$Node=="Node99",]
anc_seq1 <- paste(state1$State, collapse = "")
names(anc_seq1)<- "outbreak_Hondius_anc"

state2 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state2<- state2[state2$Node=="Node98",]
anc_seq2 <- paste(state2$State, collapse = "")
names(anc_seq2)<- "outbreak_Hondius_ref"

state3 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state3<- state3[state3$Node=="Node102",]
anc_seq3 <- paste(state3$State, collapse = "")
names(anc_seq3)<- "outbreak_PV_anc"

state4 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state4<- state4[state4$Node=="Node101",]
anc_seq4 <- paste(state4$State, collapse = "")
names(anc_seq4)<- "outbreak_PV_ref"

state5 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state5<- state5[state5$Node=="Node134",]
anc_seq5 <- paste(state5$State, collapse = "")
names(anc_seq5)<- "outbreak_MN_anc"

state6 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state6<- state6[state6$Node=="Node103",]
anc_seq6 <- paste(state6$State, collapse = "")
names(anc_seq6)<- "outbreak_MN_ref"

state7 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state7<- state7[state7$Node=="Node107",]
anc_seq7 <- paste(state7$State, collapse = "")
names(anc_seq7)<- "outbreak_epyugen_anc"

state8 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state8<- state8[state8$Node=="Node106",]
anc_seq8 <- paste(state8$State, collapse = "")
names(anc_seq8)<- "outbreak_epyugen_ref"

state9 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state9<- state9[state9$Node=="Node82",]
anc_seq9 <- paste(state9$State, collapse = "")
names(anc_seq9)<- "outbreak_KC_anc"

state10 <- read.table("./sequences_M_ex_lab_cells_concat_2026_05_27v6.cds.linsi.fasta.state", header = TRUE, stringsAsFactors = FALSE)
state10<- state10[state10$Node=="Node81",]
anc_seq10 <- paste(state10$State, collapse = "")
names(anc_seq10)<- "outbreak_KC_ref"
...
writeLines(
  ...
  "../ancestral_sequences/ancestral_sequences_M.fasta"
)


# Read fasta containing ancestral sequences aligned to reference sequence of Orthohantavirus Andes NC_003467.2
fasta <- readDNAStringSet("../ancestral_sequences_aligned/ancestral_sequences_M_aligned_to_RefSeq.fasta")
...
anc_seq9 <- as.character(fasta["outbreak_KC_anc"])
anc_seq10 <- as.character(fasta["outbreak_KC_ref"])

#Codon start and end positions assigned with respect to reference
syn_nonsyn_table <- function(ref_seq, query_seq, cds_start = 52, cds_end = 3468) {
...
mut1 <- syn_nonsyn_table(anc_seq2, anc_seq1, cds_start = 52, cds_end = 3468)
mut2 <- syn_nonsyn_table(anc_seq4, anc_seq3, cds_start = 52, cds_end = 3468)
mut3 <- syn_nonsyn_table(anc_seq6, anc_seq5, cds_start = 52, cds_end = 3468)
mut4 <- syn_nonsyn_table(anc_seq8, anc_seq7, cds_start = 52, cds_end = 3468)
mut5 <- syn_nonsyn_table(anc_seq10, anc_seq9, cds_start = 52, cds_end = 3468)
...
mut5$outbreak <- "KC_outbreak"
all_mut <- rbind(mut1, mut2, mut3, mut4, mut5)

write.csv(all_mut,"./../mutations list/all_mutations_M_segment.csv", row.names=FALSE)
